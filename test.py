...
def updater(filename, newData, newHeaders):
    ...
    priceChange = newData[totalIndex] if len(existingData) == 0 else newData[totalIndex] - Decimal(existingData[-1][totalIndex])
    percentageChange = 0 if len(existingData) == 0 or Decimal(existingData[-1][totalIndex]) == 0 else (priceChange / Decimal(existingData[-1][totalIndex])) * 100
...
async def download_all_stocks():
    async def download(url: str, stock: Stock):
        timeout = aiohttp.ClientTimeout(total=600)
        async with aiohttp.ClientSession(timeout=timeout) as s:
            resp = await s.get(url)
            read_value = await resp.read();
            soup = BeautifulSoup(read_value, 'html.parser')
    ...
    for stock in stocks:
        for exchange_type in exchange_types:
            urls[base_url + stock.name + ":" + exchange_type] = stock
    return await asyncio.gather(*[download(url, urls[url]) for url in urls])
